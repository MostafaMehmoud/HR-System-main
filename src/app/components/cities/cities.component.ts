...
export class CitiesComponent implements OnInit {
  configurationName = 'Dashboard.مدينه'
  NameAr = 'cityNameAr'
  NameEn = 'cityNameEn'
  ...
  getAllcities() {
    this.citiesService.getAllCities().subscribe({
      next: (res) => {
        ...
        this.cities.forEach((obj: any) => obj.checked = false);
        this.originalAllItems = this.cities
      },
      error: (err) => console.error(err)
    });
  }
  ...
  onSearchChange(): void {
    ...
    if (!value) {
      this.cities = this.originalAllItems
      this.filteredArr = [];
      return;
    }
  ...
  groupBy(groupBy: string) {
    if (groupBy == 'id') {
      this.cities = this.cities.sort((a, b) => Number(a.id) - Number(b.id))
    } else {
      this.cities = this.cities.sort((a: any, b: any) => this.currentLang == 'ar' ? a.nameAr.localeCompare(b.nameAr) : a.nameEn.localeCompare(b.nameEn));
    }
  }
  ...
  addEdit() {
    ...
    if (this.configurationForm.valid) {
      ...
      if (this.addOrEdit == 'add') {
        this.citiesService.addCity(this.configurationForm.value).subscribe({
          next: res => {
            this.wrongError = false
            this._spinner.hide('spinner2')
            console.log(res);
            if (res.success) {
              this._toaster.success(this.translate.instant('CITIES.SuccessTitle'));
              this.textError = ''
              this.modalCloseBtn.nativeElement.click();
              this.getAllcities();
            } else {
              this.textError = this.translate.instant('CITIES.ErrorMessage')
            }
          },
          error: err => {
            this.textError = ''
        ...
        let body = {
          'id': this.id,
          'cityNameAr': this.configurationForm.value.cityNameAr,
          'cityNameEn': this.configurationForm.value.cityNameEn,
          'companyId': this.configurationForm.value.companyId,
        }
        this.citiesService.updateCity(body).subscribe({
          next: (res) => {
            this.wrongError = false
            this._spinner.hide('spinner2')
            if (res.success) {
              this._toaster.success(this.translate.instant('CITIES.updated'));
              this.textError = ''
              this.getAllcities();
              this.modalCloseBtn.nativeElement.click();
            } else {
              this.textError = this.translate.instant('CITIES.ErrorMessage2')
            }
          },
          error: (err) => {
            this.wrongError = true
  ...
  confirmDelete(id: number) {
    Swal.fire({
      title: this.translate.instant('CITIES.DeleteConfirmTitle'),
      text: this.translate.instant('CITIES.DeleteConfirm'),
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        this.citiesService.deleteCity(id.toString()).subscribe({
          next: (res) => {
            console.log(res);
            if (res.success) {
              this.getAllcities();
              Swal.fire(
                ...
                this.translate.instant('CITIES.Success'),
                'success'
              );
            } else {
              Swal.fire(
                this.translate.instant('COMMON.Error'),
                this.translate.instant('CITIES.Error'),
                'error'
              );
            }
          },
  ...
  isAnyChecked() {
    return this.cities.some((item: any) => item.checked);
  }
  ...
  // DeleteGroup() {
  //   let ids = this.cities.filter((obj) => obj.checked).map(item => item.id)
  //   this.DeleteItem(ids)
  // }

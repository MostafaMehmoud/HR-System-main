...
export class ManagementComponent implements OnInit {
  configurationName = 'Dashboard.الادارة'
  NameAr = 'manageNameAr'
  NameEn = 'manageNameEn'
  ...
  constructor(
    ...
    this.configurationForm = this._FormBuilder.group({
      manageNameAr: ['', Validators.required],
      manageNameEn: ['', Validators.required]
    });
  }
  ...
  onSearchChange(): void {
    ...
    if (!value) {
      this.managements = this.originalAllItems
      this.filteredArr = [];
      return;
    }
  ...
  groupBy(groupBy: string) {
    if (groupBy == 'id') {
      this.managements = this.managements.sort((a, b) => Number(a.id) - Number(b.id))
    } else {
      this.managements = this.managements.sort((a: any, b: any) => this.currentLang == 'ar' ? a.nameAr.localeCompare(b.nameAr) : a.nameEn.localeCompare(b.nameEn));
    }
  }
  ...
  addEdit() {
    ...
    if (this.configurationForm.valid) {
      ...
      if (this.addOrEdit == 'add') {
        this.managementService.addManage(this.configurationForm.value).subscribe({
          next: res => {
            this.wrongError = false
            this._spinner.hide('spinner2')
            console.log(res);
            if (res.success) {
              this._toaster.success(this.translate.instant('MANAGEMENTS.SuccessTitle'));
              this.textError = ''
              this.addEditComponent?.close();
              this.getAllManage();
            } else {
              this.textError = this.translate.instant('MANAGEMENTS.ErrorMessage')
            }
          },
          error: err => {
            this.textError = ''
        ...
        let body = {
          'id': this.id,
          'manageNameAr': this.configurationForm.value.manageNameAr,
          'manageNameEn': this.configurationForm.value.manageNameEn,
        }
        this.managementService.updateManage(body).subscribe({
          next: (res) => {
            this.wrongError = false
            this._spinner.hide('spinner2')
            if (res.success) {
              this._toaster.success(this.translate.instant('MANAGEMENTS.updated'));
              this.textError = ''
              this.getAllManage();
              this.addEditComponent?.close();
            } else {
              this.textError = this.translate.instant('MANAGEMENTS.ErrorMessage2')
            }
          },
          error: (err) => {
            this.wrongError = true
  ...
  getUpdateData(item: any) {
    ...
    this.configurationForm.patchValue({
      manageNameAr: item.nameAr, // Adjust if necessary based on your response
      manageNameEn: item.nameEn  // Adjust if necessary based on your response
    });
    this.id = item.id
    this.submited = false;
    this.wrongError = false;
  ...
  confirmDelete(id: number) {
    Swal.fire({
      title: this.translate.instant('MANAGEMENTS.DeleteConfirmTitle'),
      text: this.translate.instant('MANAGEMENTS.DeleteConfirm'),
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        this.managementService.deleteManage(id.toString()).subscribe({
          next: (res) => {
            console.log(res);
            if (res.success) {
              this.getAllManage();
              Swal.fire(
                ...
                this.translate.instant('MANAGEMENTS.Success'),
                'success'
              );
            } else {
              Swal.fire(
                this.translate.instant('COMMON.Error'),
                this.translate.instant('MANAGEMENTS.Error'),
                'error'
              );
            }
          },
  ...
  isAnyChecked() {
    return this.managements.some((item: any) => item.checked);
  }
  ...
  // DeleteGroup() {
  //   let ids = this.managements.filter((obj) => obj.checked).map(item => item.id)
  //   this.DeleteItem(ids)
  // }

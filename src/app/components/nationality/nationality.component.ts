...
@Component({
  ...
  styleUrls: ['./nationality.component.css' , '../../shared/configration.css']
})

export class NationalityComponent implements OnInit {
  configurationName = 'Dashboard.الجنسية'
  NameAr = 'nationNameAr'
  NameEn = 'nationNameEn'
  ...
  constructor(
    ...
    this.configurationForm = this._FormBuilder.group({
      nationNameAr: ['', Validators.required],
      nationNameEn: ['', Validators.required]
    });
  }
  ...
  getAllNationalities() {
    this.nationalityService.getAllNation().subscribe({
      next: (res) => {
        this.Nationalities = res.data
        if(this.Nationalities.length > 0){
          this.Nationalities.forEach((obj: any) => obj.checked = false);
        }
        this.originalAllItems = this.Nationalities
      },
  ...
  onSearchChange(): void {
    ...
    if (!value) {
      this.Nationalities = this.originalAllItems
      this.filteredArr = [];
      return;
    }
  ...
  groupBy(groupBy: string) {
    if (groupBy == 'id') {
      this.Nationalities = this.Nationalities.sort((a, b) => Number(a.id) - Number(b.id))
    } else {
      this.Nationalities = this.Nationalities.sort((a: any, b: any) => this.currentLang == 'ar' ? a.nameAr.localeCompare(b.nameAr) : a.nameEn.localeCompare(b.nameEn));
    }
  }
  ...
  addEdit() {
    ...
    if (this.configurationForm.valid) {
      ...
      if (this.addOrEdit == 'add') {
        this.nationalityService.addNation(this.configurationForm.value).subscribe({
          next: res => {
            this.wrongError = false
            this._spinner.hide('spinner2')
            console.log(res);
            if (res.success) {
              this._toaster.success(this.translate.instant('NATIONALITIES.SuccessTitle'));
              this.textError = ''
              this.addEditComponent?.close();
              this.getAllNationalities();
            } else {
              this.textError = this.translate.instant('NATIONALITIES.ErrorMessage')
            }
          },
          error: err => {
            this.textError = ''
        ...
        let body = {
          'id': this.id,
          'nationNameAr': this.configurationForm.value.nationNameAr,
          'nationNameEn': this.configurationForm.value.nationNameEn,
        }
        this.nationalityService.updateNation(body).subscribe({
          next: (res) => {
            this.wrongError = false
            this._spinner.hide('spinner2')
            if (res.success) {
              this._toaster.success(this.translate.instant('NATIONALITIES.updated'));
              this.textError = ''
              this.getAllNationalities();
              this.addEditComponent?.close();
            } else {
              this.textError = this.translate.instant('NATIONALITIES.ErrorMessage2')
            }
          },
          error: (err) => {
            this.wrongError = true
  ...
  getUpdateData(item: any) {
    ...
    this.configurationForm.patchValue({
      nationNameAr: item.nameAr, // Adjust if necessary based on your response
      nationNameEn: item.nameEn  // Adjust if necessary based on your response
    });
    this.id = item.id
    this.submited = false;
    this.wrongError = false;
  ...
  confirmDelete(id: number) {
    Swal.fire({
      title: this.translate.instant('NATIONALITIES.DeleteConfirmTitle'),
      text: this.translate.instant('NATIONALITIES.DeleteConfirm'),
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        this.nationalityService.deleteNation(id.toString()).subscribe({
          next: (res) => {
            console.log(res);
            if (res.success) {
              this.getAllNationalities();
              Swal.fire(
                ...
                this.translate.instant('NATIONALITIES.Success'),
                'success'
              );
            } else {
              Swal.fire(
                this.translate.instant('COMMON.Error'),
                this.translate.instant('NATIONALITIES.Error'),
                'error'
              );
            }
          },
  ...
  isAnyChecked() {
    return this.Nationalities.some((item: any) => item.checked);
  }
  ...
  // DeleteGroup() {
  //   let ids = this.Nationalities.filter((obj) => obj.checked).map(item => item.id)
  //   this.DeleteItem(ids)
  // }

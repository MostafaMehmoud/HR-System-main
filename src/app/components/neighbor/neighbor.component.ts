...
import { NeighborService } from 'src/app/shared/services/neighbor/neighbor.service';
...
export class NeighborComponent implements OnInit {
  configurationName = 'Dashboard.قرابه'
  NameAr = 'neighborNameAr'
  NameEn = 'neighborNameEn'
  ...
  constructor(
    ...
    this.configurationForm = this._FormBuilder.group({
      neighborNameAr: ['', Validators.required],
      neighborNameEn: ['', Validators.required]
    });
  }
  ...
  getAllneighbors() {
    this.neighborService.getAllNeighbor().subscribe({
      next: (res) => {
        this.neighbors = res.data
        if(this.neighbors.length > 0){
          this.neighbors.forEach((obj: any) => obj.checked = false);
        }
        this.originalAllItems = this.neighbors
      },
  ...
  onSearchChange(): void {
    ...
    if (!value) {
      this.neighbors = this.originalAllItems
      this.filteredArr = [];
      return;
    }
  ...
  groupBy(groupBy: string) {
    if (groupBy == 'id') {
      this.neighbors = this.neighbors.sort((a, b) => Number(a.id) - Number(b.id))
    } else {
      this.neighbors = this.neighbors.sort((a: any, b: any) => this.currentLang == 'ar' ? a.nameAr.localeCompare(b.nameAr) : a.nameEn.localeCompare(b.nameEn));
    }
  }
  ...
  addEdit() {
    ...
    if (this.configurationForm.valid) {
      ...
      if (this.addOrEdit == 'add') {
        this.neighborService.addNeighbor(this.configurationForm.value).subscribe({
          next: res => {
            this.wrongError = false
            this._spinner.hide('spinner2')
            console.log(res);
            if (res.success) {
              this._toaster.success(this.translate.instant('NEIGHBORS.SuccessTitle'));
              this.textError = ''
        ...
        let body = {
          'id': this.id,
          'neighborNameAr': this.configurationForm.value.neighborNameAr,
          'neighborNameEn': this.configurationForm.value.neighborNameEn,
        }
        this.neighborService.updateNeighbor(body).subscribe({
          next: (res) => {
            this.wrongError = false
            this._spinner.hide('spinner2')
            if (res.success) {
              this._toaster.success(this.translate.instant('NEIGHBORS.updated'));
              this.textError = ''
              this.getAllneighbors();
              this.addEditComponent?.close();
            } else {
  ...
  getUpdateData(item: any) {
    ...
    this.configurationForm.patchValue({
      neighborNameAr: item.nameAr, // Adjust if necessary based on your response
      neighborNameEn: item.nameEn  // Adjust if necessary based on your response
    });
    this.id = item.id
    this.submited = false;
    this.wrongError = false;
  ...
  confirmDelete(id: number) {
    Swal.fire({
      title: this.translate.instant('NEIGHBORS.DeleteConfirmTitle'),
      text: this.translate.instant('NEIGHBORS.DeleteConfirm'),
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        this.neighborService.deleteNeighbor(id.toString()).subscribe({
          next: (res) => {
            console.log(res);
            if (res.success) {
              this.getAllneighbors();
              Swal.fire(
                ...
                this.translate.instant('NEIGHBORS.Success'),
                'success'
              );
            } else {
              Swal.fire(
                this.translate.instant('COMMON.Error'),
                this.translate.instant('NEIGHBORS.Error'),
                'error'
              );
            }
          },
  ...
  isAnyChecked() {
    return this.neighbors.some((item: any) => item.checked);
  }
  ...
  // DeleteGroup() {
  //   let ids = this.neighbors.filter((obj) => obj.checked).map(item => item.id)
  //   this.DeleteItem(ids)
  // }

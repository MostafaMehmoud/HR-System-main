...
import { ReligionsService } from 'src/app/shared/services/religions/religions.service';
...
export class ReligionsComponent implements OnInit {
  configurationName = 'Dashboard.ديانة'
  NameAr = 'religionNameAr'
  NameEn = 'religionNameEn'
  ...
  constructor(
    ...
    this.configurationForm = this._FormBuilder.group({
      religionNameAr: ['', Validators.required],
  ...
  getAllreligions() {
    this.religionsService.getAllReligion().subscribe({
      next: (res) => {
        this.religions = res.data
        if(this.religions.length > 0){
          this.religions.forEach((obj: any) => obj.checked = false);
        }
        this.originalAllItems = this.religions
      },
  ...
  onSearchChange(): void {
    ...
    if (!value) {
      this.religions = this.originalAllItems
      this.filteredArr = [];
      return;
    }
  ...
  groupBy(groupBy: string) {
    if (groupBy == 'id') {
      this.religions = this.religions.sort((a, b) => Number(a.id) - Number(b.id))
    } else {
      this.religions = this.religions.sort((a: any, b: any) => this.currentLang == 'ar' ? a.nameAr.localeCompare(b.nameAr) : a.nameEn.localeCompare(b.nameEn));
    }
  }
  ...
  addEdit() {
    ...
    if (this.configurationForm.valid) {
      ...
      if (this.addOrEdit == 'add') {
        this.religionsService.addReligion(this.configurationForm.value).subscribe({
          next: res => {
            this.wrongError = false
            this._spinner.hide('spinner2')
            console.log(res);
            if (res.success) {
              this._toaster.success(this.translate.instant('RELIGIONS.SuccessTitle'));
              this.textError = ''
              this.addEditComponent?.close();
              this.getAllreligions();
            } else {
              this.textError = this.translate.instant('RELIGIONS.ErrorMessage')
            }
          },
          error: err => {
            this.textError = ''
        ...
        let body = {
          'id': this.id,
          'religionNameAr': this.configurationForm.value.religionNameAr,
          'religionNameEn': this.configurationForm.value.religionNameEn,
        }
        this.religionsService.updateReligion(body).subscribe({
          next: (res) => {
            this.wrongError = false
            this._spinner.hide('spinner2')
            if (res.success) {
              this._toaster.success(this.translate.instant('RELIGIONS.updated'));
              this.textError = ''
              this.getAllreligions();
              this.addEditComponent?.close();
            } else {
              this.textError = this.translate.instant('RELIGIONS.ErrorMessage2')
            }
          },
          error: (err) => {
            this.wrongError = true
  ...
  getUpdateData(item: any) {
    ...
    this.configurationForm.patchValue({
      religionNameAr: item.nameAr, // Adjust if necessary based on your response
      religionNameEn: item.nameEn  // Adjust if necessary based on your response
    });
    this.id = item.id
    this.submited = false;
    this.wrongError = false;
  ...
  confirmDelete(id: number) {
    Swal.fire({
      title: this.translate.instant('RELIGIONS.DeleteConfirmTitle'),
      text: this.translate.instant('RELIGIONS.DeleteConfirm'),
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        this.religionsService.deleteReligion(id.toString()).subscribe({
          next: (res) => {
            console.log(res);
            if (res.success) {
              this.getAllreligions();
              Swal.fire(
                ...
                this.translate.instant('RELIGIONS.Success'),
                'success'
              );
            } else {
              Swal.fire(
                this.translate.instant('COMMON.Error'),
                this.translate.instant('RELIGIONS.Error'),
                'error'
              );
            }
          },
  ...
  isAnyChecked() {
    return this.religions.some((item: any) => item.checked);
  }
  ...
  // DeleteGroup() {
  //   let ids = this.religions.filter((obj) => obj.checked).map(item => item.id)
  //   this.DeleteItem(ids)
  // }
